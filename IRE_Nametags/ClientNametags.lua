...
g_fMaxDistance		= 8.0;
g_pBubbleManager	= new. BubbleManager();

font = dxCreateFont( "arialbd.ttf", 10, true );
...
function Draw( pPlayer, bPed )
	if pPlayer.GetDimension() == g_iDimension and pPlayer.GetInterior() == g_iInterior and pPlayer.IsOnScreen() and pPlayer.GetAlpha() > 64 then
		if not pPlayer.GetData( "Player::Nametag::Showing" ) then
			return;
		end
		...
		if fDistance <= g_fMaxDistance and g_vecPosition.IsLineOfSightClear( vecBonePosition, true, false, false, true, false, false, true ) then
			...
			if fScreenX and fScreenY then
				...
				local sText		= pPlayer.GetData( "Player::Nametag::Text" );
				
				local iColor3	= tocolor( 0, 0, 0, fAlpha );
				
				if sText then					
					local color = pPlayer.GetData( "Player::Nametag::Color" );
					...
					dxDrawText( sText, fScreenX + 1, fScreenY + 1, fScreenX + 1, fScreenY + 1, iColor3, 1.0, font, 'center', 'bottom' );
					dxDrawText( sText, fScreenX - 1, fScreenY - 1, fScreenX - 1, fScreenY - 1, iColor3, 1.0, font, 'center', 'bottom' );
					dxDrawText( sText, fScreenX + 1, fScreenY - 1, fScreenX + 1, fScreenY - 1, iColor3, 1.0, font, 'center', 'bottom' );
					dxDrawText( sText, fScreenX - 1, fScreenY + 1, fScreenX - 1, fScreenY + 1, iColor3, 1.0, font, 'center', 'bottom' );
					dxDrawText( sText, fScreenX, fScreenY, fScreenX, fScreenY, iColor1, 1.0, font, 'center', 'bottom' );
				end
...
addEventHandler( 'onClientRender', root, 
	function()
		g_fMaxDistance		= tonumber( CLIENT.GetData( 'Player::Nametag::MaxDistance' ) ) or 8.0;
